...
//    }
    ...
    public synchronized void addSuggestions(List<Object> suggestions) {
        ArrayList<ContentValues> toSave = new ArrayList<>();
        for (Object str : suggestions) {
            ContentValues value = new ContentValues();
            value.put(HistoryContract.HistoryEntry.COLUMN_QUERY, str.toString());
            value.put(HistoryContract.HistoryEntry.COLUMN_INSERT_DATE, System.currentTimeMillis());
            value.put(HistoryContract.HistoryEntry.COLUMN_IS_HISTORY, 0); // Saving as suggestion.
            value.put(HistoryContract.HistoryEntry.COLUMN_JSON_OBJECT, new Gson().toJson(str));
    ...
    public void addSuggestions(Object[] suggestions) {
        ArrayList<Object> list = new ArrayList<>(Arrays.asList(suggestions));
        addSuggestions(list);
    }
